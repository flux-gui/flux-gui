...
This page provides guides for configuring Flux-GUI with the most common OIDC providers.

## Google

Google's identity provider does not support the groups scope which Flux-GUI requests by default. That's why in
...
there is a [dedicated guide on setting up Keycloak and Flux-GUI to work together](./configuring-oidc-with-keycloak.mdx).
