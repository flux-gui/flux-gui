...
In this guide we will show you how to enable users to login to the Flux-GUI dashboard by authenticating them against a Keycloak instance.

This example uses [Keycloak](https://www.keycloak.org/) and assumes Flux-GUI has already been installed on the Kubernetes Cluster.

## Pre-requisites

- A Kubernetes cluster
- Flux-GUI is [installed](../../intro-weave-gitops/#getting-started) and [TLS has been enabled](../../enterprise/getting-started/install-enterprise/#tls-configuration)
...
1. Set the `Home URL` for your client to the URL of your Flux-GUI instance. For this demo, that's `https://WEAVE_GITOPS_URL`
1. Set the `Valid redirect URIs` to the URL of your Flux-GUI instance followed by `/oauth2/callback`. For this demo, that's `https://WEAVE_GITOPS_URL/oauth2/callback`
1. Click on `Save`
...
Now that everything is configured, we need to grab the client secret from Keycloak in order to configure OIDC for Flux-GUI.
...
## Configuring Flux-GUI to use our new Keycloak Client

Now that our Keycloak configuration is done, it's time to link our Flux-GUI deployment to it.

### Creating the `oidc-auth` secret

To configure Flux-GUI for OIDC authentication via Keycloak, we need to configure the `oidc-auth` secret. We need to set:

- the `issuerURL` to the URL of our keycloak instance, followed by `/realms/<realm_name>`
- the `redirectURL` to the URL of our Flux-GUI instance, followed by `/oauth2/callback`
...
After this secret is created, you may need to delete the Flux-GUI pods in order to restart the app and load the new config.

### Setting up RBAC

Once Flux-GUI is configured for OIDC, we need a way to map permissions to the groups. To do that, we need to create role bindings for our `wego-admin` group.
...
Navigate to your Flux-GUI dashboard URL and click on the `Login With Keycloak` button. You should be redirected to a Keycloak Login page.
...
You should now be redirected back to Flux-GUI and, thanks to the RBAC configuration, you should now see all of the configured applications.
