...
Flux-GUI Enterprise provides monitoring telemetry and tooling for metrics and [profiling](#profiling). WGE generates [Prometheus](https://prometheus.io/) metrics for monitoring both performance and business operations.
...
spec:
  ...
  ref:
    branch: main
  url: https://github.com/flux-gui/flux-gui-quickstart
---
apiVersion: v1
kind: Namespace
metadata:
...
3. Deploy Flux-GUI Monitoring Config
...
Monitor Flux-GUI golden signals for API server and controllers:

![weave gitops dashboard list](imgs/monitoring-dashboard-overview.png)

**Flux-GUI Runtime**

Monitor Flux-GUI Go runtime metrics like memory usage, memory heap, and Goroutines, among others.
...
During operations, profiling is useful for gaining a deeper understanding of how Flux-GUI runtime behaves.
Given that Flux-GUI is written in Go, profiling happens through [pprof](https://pkg.go.dev/runtime/pprof). It is
