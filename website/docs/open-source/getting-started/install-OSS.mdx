---
title: Step 1 - Install Flux-GUI Open Source
hide_title: true
pagination_next: open-source/getting-started/ui-OSS
---

# Step 1: Install Flux-GUI Open Source on Your Cluster

:::tip
These instructions only apply to Flux-GUI Open Source. To install Flux-GUI Enterprise, [go here](../../enterprise/getting-started/install-enterprise.mdx).
:::

This page covers Flux-GUI Open Source installation and is adapted from the [Flux - Getting Started](https://fluxcd.io/docs/get-started/) guide.

If you haven't already, please check out our [Introduction to Flux-GUI](../../intro-weave-gitops.mdx) page for additional information about Flux-GUI Open Source as well as our Enterprise version.

## Prerequisites

Before you can install Flux-GUI Open Source, you will need:
...
No matter which version of Flux-GUI you install, you need to have a Kubernetes cluster up
and running. We test Flux-GUI against the latest [supported Kubernetes releases](https://kubernetes.io/releases/).
...
Flux-GUI is an extension to Flux. Therefore, it requires that Flux 0.32 or a
...
Flux-GUI includes a command-line interface to help users create and manage resources. The `gitops` CLI is currently supported on Mac (x86 and Arm) and Linux, including Windows Subsystem for Linux (WSL). Windows support is a [planned enhancement](https://github.com/flux-gui/flux-gui/issues/663).
...
```bash
curl --silent --location "https://github.com/flux-gui/flux-gui/releases/download/v0.38.0/gitops-$(uname)-$(uname -m).tar.gz" | tar xz -C /tmp
...
Run the following command, which will create a `HelmRepository` and `HelmRelease` to deploy Flux-GUI:
...
You will use the password you've just created when you've finished Flux-GUI Open Source installation and are ready to login to the dashboard UI.
...
:::tip
If you need to customize the Flux-GUI Helm release, you can use the `--values` CLI flag to supply one or more values files.
...
#### Commit and push the `weave-gitops-dashboard.yaml` to the `fleet-infra` repository
   ...
   ```
   git add -A && git commit -m "Add Flux-GUI Dashboard"
   git push
   ```

#### Validate that Flux-GUI and Flux are installed
...
If you wait for a while and still nothing happens, it might be that your manifests haven’t been exported to the repository. This means that Flux-GUI won't install.

:::tip
You can use the Flux-GUI Helm Chart to customize your installation.
Find the full Chart reference [here](../../references/helm-reference.md).
:::

## Supercharge your Flux-GUI

You could optionally unlock more from Flux-GUI by installing other components:
...
Now let's [explore the Flux-GUI Open Source UI](./ui-OSS.mdx). Then, we'll deploy an application.
