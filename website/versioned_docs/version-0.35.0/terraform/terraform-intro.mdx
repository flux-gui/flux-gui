...
[Terraform Controller](https://github.com/weaveworks/tf-controller) (TF-Controller) is a reliable tool for managing your infrastructure and application resources using the GitOps approach, all at your own pace. An open source project created by Weaveworks, the makers of [Flux](https://fluxcd.io), TF-Controller follows patterns established by Flux and integrates with Flux-GUI. 
...
With Flux-GUI Enterprise, you can manage `Terraform` objects the same way you can with `Kustomization` and `HelmReleases`:
...
TF-controller has its own versioning system that is separate from the versioning system used by Flux-GUI.
This means that you can install and use TF-controller independently of Flux-GUI—it will not be affected
by the version of Flux-GUI that you are using.
