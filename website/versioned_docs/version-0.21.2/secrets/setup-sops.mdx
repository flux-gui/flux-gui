...
OpenPGP is a way of using SOPS to encrypt and decrypt secrets with Flux-GUI Enterprise.

Here are the steps to generate an OpenPGP key and configure your cluster to work with Flux-GUI Enterprise secrets management.
...
4- Export the generated public key to a kubernetes secret `sops-gpg-public-key` which will be used by Flux-GUI Enterprise to encrypt the secrets created from the UI.
...
[age](https://github.com/FiloSottile/age) is a simple, modern and secure file encryption tool, that can be used to encrypt secrets using Flux-GUI Enterprise.

Here are the steps to generate an age key and configure your cluster to work with Flux-GUI Enterprise secrets management.
...
4- Export the generated public key to a kubernetes secret `sops-age-public-key` which will be used by Flux-GUI Enterprise to encrypt the secrets created from the UI.
