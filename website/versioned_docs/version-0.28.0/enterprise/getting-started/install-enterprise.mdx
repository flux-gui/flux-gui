...
To get you started, we'll cover EKS as our management cluster with the CAPA provider. Please note again that Flux-GUI Enterprise supports [clusters without Cluster API](../../cluster-management/managing-clusters-without-capi.mdx), as well as any combination of management cluster and CAPI provider.
...
You do not need to install a CAPI provider to provision Kubernetes clusters using Flux-GUI Enterprise—you can also provision with Terraform. But for this example with CAPA, you must.
