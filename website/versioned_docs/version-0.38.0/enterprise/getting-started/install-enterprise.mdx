---
title: Install Flux-GUI Enterprise
hide_title: true
...
# Install Flux-GUI Enterprise<TierLabel tiers="Enterprise" />
f
:::info
To purchase an entitlement to Flux-GUI Enterprise, please contact [info@weaveworks.org](mailto:info@weaveworks.org).
:::
...
:::tip
There is no need to install the open source version of Flux-GUI before installing Flux-GUI Enterprise.
:::

## Prerequisites

To get up and running with Flux-GUI Enterprise:
...
### Install the Flux-GUI Enterprise CLI Tool
...
You may decide to give your engineering teams access to the WGE dashboard so they can view and manage their workloads. In this case, you will want to secure dashboard access and restrict who can interact with it. Flux-GUI Enterprise integrates with your OIDC provider and uses standard Kubernetes RBAC to give you fine-grained control of the dashboard users' permissions.

OIDC extends the OAuth2 authorization protocol by including an additional field (ID Token) that contains information (claims) about a user's identity. After a user successfully authenticates with the OIDC provider, Flux-GUI Enterprise uses this information to impersonate the user in any calls to the Kubernetes API. This allows cluster administrators to use RBAC rules to control access to the cluster and the dashboard.

For more specific examples of how to setup OIDC with Flux-GUI, see [this guide](../../../guides/oidc/).

If you run into issues setting up OIDC with Flux-GUI, please refer to the [`gitops check oidc-config` command](../../../references/cli-reference/gitops_check_oidc-config/)
...
| `clientID`        |  The client ID set up for Flux-GUI in the issuer                                                                 |           |
| `clientSecret`    |  The client secret set up for Flux-GUI in the issuer                                                             |           |
| `redirectURL`     |  The redirect URL set up for Flux-GUI in the issuer—typically the dashboard URL, followed by `/oauth2/callback ` |           |
...
This example uses [Dex](https://dexidp.io/) and its [GitHub connector](https://dexidp.io/docs/connectors/github/) to show you how to log in to the Flux-GUI dashboard by authenticating with your GitHub account. It assumes you have already installed Flux-GUI on a Kubernetes cluster, per the instructions above, and have also [enabled TLS](#tls-configuration).
...
Note also that this mechanism only exists for a single user. You will not be able to create multiple users. Flux-GUI does not provide its own authentication mechanism. For secure and fully-featured authentication we **strongly recommend** using an OIDC provider, as described in the other tab.
:::
...
for the Flux-GUI application, which the WGE UI requires to work. The default permissions will generate a cluster role that includes the permissions:
...
The primary way Flux-GUI queries the Kube API is via `impersonation`. The permissions granted to users and groups that Flux-GUI
...
needed by Flux-GUI application users and groups. At a minimum, a User should be bound to a Role in the `flux-system` namespace—which is where
...
| infra.contrib.fluxcd.io        | [Terraform](https://github.com/weaveworks/tf-controller)                                                             | get, list, patch |
 ...
 Flux-GUI needs to be able to query the [CRDs](https://fluxcd.io/docs/components/) that Flux uses before it can accurately display Flux state. The
...
Flux-GUI reads basic resources so that it can monitor the effect that Flux has
on what's running.

Reading `secrets` enables Flux-GUI to monitor the state of Helm releases
...
Adjust the configuration in the Helm `values.yaml` file or the `spec.values` section of the Flux-GUI `HelmRelease` resource:
...
Our general recommendation is to use OIDC and a small number of groups that Flux-GUI can impersonate.

Configuring Flux-GUI to impersonate Kubernetes groups rather than users has the following benefits:
* A user's permissions for impersonation by Flux-GUI can be separate from
...
* Aisha, a cluster admin, who should have full admin access to Flux-GUI
* Brian, lead of Team-A, who should have admin permissions to their team's
  namespace in Flux-GUI and read-only otherwise
* June and Jo, developers in Team-A who should have read-only access to Flux-GUI
...
:::caution Using OIDC for cluster and Flux-GUI Authentication
If the same OIDC provider is used to authenticate a user with the cluster
itself (e.g. for use with `kubectl`) and to Flux-GUI then, depending
on OIDC configuration, they may end up with the super-set of their permissions
from Flux-GUI and any other permissions granted to them.
...
to which clients. The Flux-GUI groups should not be returned to the
...
## Configure Access for Writing to Git from the Flux-GUI Enterprise UI
...
git commit -m "Deploy Flux-GUI Enterprise"
git push
```
