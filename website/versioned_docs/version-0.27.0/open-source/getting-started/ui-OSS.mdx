...
# Step 2: Explore the Flux-GUI Open Source UI

The Flux-GUI user interface enables you to manage and view all of your applications in one place. This documentation gives you an overview of the Flux-GUI Open Source UI.

:::tip
To check out Flux-GUI Enterprise's UI, which provides an even richer user experience, please contact info@weaveworks.org.
:::

## Overview

A quick preview of what the Flux-GUI Open Source UI provides:
...
* A **Notifications View** that makes it easy to understand which notifications are already configured within the UI, so that you can simply set up and receive notifications from Flux-GUI. If you’re a platform operator, this view will help you to understand your egress topology across clusters so you’ll know where events are being sent beyond your clusters.
...
In the above screenshot you can see:
- a `GitRepository` called `flux-system`, which was created when Flux was bootstrapped onto the cluster. It contains the manifests for the GitOps Toolkit and Flux-GUI, and also various Flux objects.
...
- a `HelmRepository` called `ww-gitops`. This pulls from the Helm Repository where the Flux-GUI Helm Chart is published.
