...
## Installing Flux-GUI on your Cluster

:::tip
These are the instructions to install the OSS tier Flux-GUI. To install Enterprise
Flux-GUI, follow the instructions [here][ee-install].
:::
...
Flux-GUI is an extension to Flux and therefore requires that Flux 0.32 or
...
```bash
curl --silent --location "https://github.com/flux-gui/flux-gui/releases/download/v0.22.0/gitops-$(uname)-$(uname -m).tar.gz" | tar xz -C /tmp
...
1. Validate that Flux-GUI and Flux are installed. _Note: this wont be instantaneous,
   ...
   ```
	...
	:::tip
	There are many other things you can configure in the Flux-GUI Helm Chart.
