...
This guide will show you how to use a template to create a Terraform resource in Flux-GUI Enterprise.

## CLI guide

### Pre-requisites
- Install [Flux-GUI Enterprise](../enterprise/getting-started/install-enterprise.mdx) with [TF-Controller installed](../enterprise/getting-started/install-enterprise.mdx#optional-install-the-terraform-controller) and [TLS enabled](../configuration/tls.md).
...
A resource can be created from a template by specifying the template's name and supplying values to it, as well as your Flux-GUI Enterprise username, password, and HTTP API endpoint.
```bash
