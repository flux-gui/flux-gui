...
You may decide to give your engineering teams access to the dashboard, in order to view and manage their workloads. In this case, you will want to secure access to the dashboard and restrict who can interact with it. Flux-GUI integrates with your OIDC provider and uses standard Kubernetes RBAC to give you fine-grained control of the permissions for the dashboard users.

#### Background

OIDC extends the OAuth2 authorization protocol by including an additional field (ID Token) that contains information (claims) about a user's identity. After a user successfully authenticates with the OIDC provider, this information is used by Flux-GUI to impersonate the user in any calls to the Kubernetes API. This allows cluster administrators to use RBAC rules to control access to the cluster and also the dashboard.
...
| `clientID`        |  The client ID that has been setup for Flux-GUI in the issuer                                                                 |           |
| `clientSecret`    |  The client secret that has been setup for Flux-GUI in the issuer                                                             |           |
| `redirectURL`     |  The redirect URL that has been setup for Flux-GUI in the issuer, typically the dashboard URL followed by `/oauth2/callback ` |           |
