...
GitOpsTemplates enables Application Developers to self-service components and services using Flux-GUI. 
...
**How do they fit today into Flux-GUI?**
...
- From values.yaml for the Flux-GUI Enterprise `mccp` chart: `values.config.capi.repositoryPath`, default: `clusters/management/clusters`
...
Flux-GUI will search for templates in the `default` namespace. This can be changed by configuring the `config.capi.namespace` value in the helm chart.
