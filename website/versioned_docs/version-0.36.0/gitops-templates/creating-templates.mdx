...
Flux-GUI will search for templates in the `default` namespace.
This can be changed by configuring the `config.capi.namespace` value in the
Flux-GUI Enterprise Helm Chart.
...
Template types are used by Flux-GUI to group the templates nicely in the
Dashboard UI.
