...
See also our [guide to installing Flux-GUI Enterprise on Azure](install-enterprise-azure.mdx):
...
These generated resources are viewable through Flux-GUI Enterprise.

Flux-GUI itself is deployed by Flux using a HelmRelease that pulls the Helm Chart. It doesn’t need to install Flux, as it is assumed that Flux is already deployed. Therefore it can use the Azure Flux add-on, which poses no conflicts with WGE itself.
