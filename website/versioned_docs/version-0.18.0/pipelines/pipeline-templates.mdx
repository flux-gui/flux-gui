...
To create new Pipelines and their required resources from within Flux-GUI Enterprise, you can leverage [GitOpsTemplates](../../gitops-templates/templates) to help platform teams scale for developer self-service. 
...
## Adding new resources from within the Flux-GUI Enterprise dashboard
GitOpsTemplates are custom resources installed onto the management cluster where Flux-GUI Enterprise resides. To add a new Pipeline, click `Create a Pipeline` from within the Pipeline view, which will take you to a pre-filtered list of templates with the label: `weave.works/template-type: pipeline`.
...
:::tip Included Sample
This template is shipped by default with Flux-GUI Enterprise to help you get started with Pipelines.
:::
...
spec:
  ...
  params:
    - name: RESOURCE_NAME # This is a required parameter name to enable Flux-GUI to write to your Git Repository
