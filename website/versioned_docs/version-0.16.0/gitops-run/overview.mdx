...
* Multiple options for debugging Flux such as using the Dashboard that comes with Flux-GUI or getting live feedback by leveraging the [GitOps Tools for Flux](https://marketplace.visualstudio.com/items?itemName=Weaveworks.vscode-gitops-tools) VSCode extension.
...
#### GitOps:
This is the default mode we are always aiming for when using Flux-GUI. Whenever GitOps Run
...
Flux-GUI Run can has two different ways of interacting with your cluster.
