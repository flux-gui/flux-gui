...
We will provide a complete walk-through of getting Flux installed and Flux-GUI configured. However, if you have:
- an existing cluster bootstrapped Flux version >= 0.32.0 🎉
- followed our [installation](./index.mdx) doc to configure access to the Flux-GUI dashboard then install Flux-GUI 👏

Then you can skip ahead to [the Flux-GUI overview](../getting-started.mdx#part-1---weave-gitops-overview) 🏃
...
Flux-GUI is an extension to Flux and therefore requires that Flux 0.32 or later has already been installed on your Kubernetes cluster. Full documentation is available at: [https://fluxcd.io/docs/installation/](https://fluxcd.io/docs/installation/).

This version of Flux-GUI is tested against the following Flux releases:
* 0.32
* 0.33
* 0.34
* 0.35
...
Flux-GUI includes a command-line interface to help users create and manage resources.
...
Windows support is a [planned enhancement](https://github.com/flux-gui/flux-gui/issues/663).
:::
...
```bash
curl --silent --location "https://github.com/flux-gui/flux-gui/releases/download/v0.16.0/gitops-$(uname)-$(uname -m).tar.gz" | tar xz -C /tmp
...
1. Run the following command which will create a `HelmRepository` and `HelmRelease` to deploy Flux-GUI
...
1. Validate that Flux-GUI and Flux are installed
