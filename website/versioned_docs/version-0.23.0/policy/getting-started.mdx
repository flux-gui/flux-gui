...
Enabling the Weave Policy Engine features in Flux-GUI is done by running the policy agent on the cluster. This section gives an overview of the policy ecosystem and the steps required for installing and running the policy agent on leaf clusters.
...
### Flux-GUI
You need to have a running instance of Flux-GUI with at least one CAPI provider installed to provision Kubernetes clusters. See [Flux-GUI Installation](https://docs.gitops.weaveworks.org/docs/installation/) page for more details about installing Flux-GUI.
...
## Policies in UI
After the leaf cluster is provisioned and the profile is installed, you should now see the policies listed in the Policies tab in Flux-GUI UI.
...
You can go to the `Violations Log` in Flux-GUI UI to view the policy violations of all the connected clusters, and dive into the details of each violation.
