...
:::tip
These instructions only apply to Flux-GUI Open Source. To install Flux-GUI Enterprise, [go here][ee-install].
:::
...
```bash
curl --silent --location "https://github.com/flux-gui/flux-gui/releases/download/v0.25.0/gitops-$(uname)-$(uname -m).tar.gz" | tar xz -C /tmp
